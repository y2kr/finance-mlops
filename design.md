...
| Decision | Resolution |
|---|---|
| **Scope of first run** | **3-class, PhraseBank `75Agree \ AllAgree` only** — the *identical* data the baseline used. Holds the data constant so the run isolates one variable: transformer fine-tune vs TF-IDF+logreg. Weak-labelled RSS + the `irrelevant` 4th class are a **separate, later run (v3)** — bundling model-swap + new noisy source + 4th class into one run would make a win/loss unattributable. One MLflow run answers one question. Also available today regardless of how much RSS has accumulated. |
| **Base model** | **`yiyanghkust/finbert-pretrain`** (finance-domain masked-LM, **no PhraseBank labels**) → fresh classification head via `AutoModelForSequenceClassification`, fine-tuned on `75Agree \ AllAgree`. **Corrects a Stage 0 leakage trap:** `ProsusAI/finbert` is itself fine-tuned on PhraseBank (and `AllAgree ⊂ 75Agree`), so using it as the base would mean the fine-tune's `AllAgree` eval grades *memorisation*, not generalisation. The clean-MLM base keeps `AllAgree` leak-free on both sides. Fallback if fiddly: `deberta-v3-small`/`distilbert-base` (clean, loses finance pretraining → "does finance pretraining help?" becomes its own follow-up). **Pin the HF `revision` (commit SHA)**, mirroring the Stage 2 Ollama-digest discipline. Pinned: `88ab954a39ea6d3ce2b62cff086dd5ad1172c664` (main, 2022-10-17; repo ships `pytorch_model.bin` + `vocab.txt` only → `BertTokenizer`). |
| **Benchmark** | **`ProsusAI/finbert` off-the-shelf, reported but NOT in the gate** — its `AllAgree` score is **train-on-test inflated** (it trained on PhraseBank), an inflated ceiling, not a clean reference. Beating it is a strong-but-noisy signal. The **honest promotion gate anchors on the clean TF-IDF floor (0.642 / 0.525)**, leak-free on both sides. (Splits the single "FinBERT = benchmark *and* base" role Stage 0 conflated.) |
| **Fine-tune method** | **Full fine-tune.** BERT-base (~110M) fits 12 GB trivially; full FT is the standard, strongest path for a small encoder. LoRA buys only a PEFT talking point on a model too small to need it, and §5's learning budget is the MLOps layer, not modelling tricks. LoRA deferred indefinitely. |
| **Validation discipline** | **Carve a stratified 90/10 validation split from the 1189 train rows** (fixed seed) for early stopping + checkpoint selection. **`AllAgree` is touched exactly once, at the final gate** — never for model selection (selecting on the gate set is the same leakage class as the base-model trap). The baseline never needed this (nothing to early-stop); a fine-tune does. |
| **Imbalance** | **Weighted cross-entropy** (inverse-frequency class weights), mirroring the baseline's `class_weight="balanced"`. PhraseBank is ~61% neutral and the gate carries a **bearish-F1 floor** (the easiest class to silently tank) — weighting keeps the comparison fair and guards that floor. |
| **Hyperparameters** | bf16 (Ada-native, stabler than fp16); `max_seq_len=128` (headlines short, zero truncation risk); batch 32; lr 2e-5, linear warmup+decay; 3–4 epochs with early stopping on **val** macro-F1; **seed fixed + logged** (GPU FT isn't bit-reproducible — log seed + `torch`/`transformers` versions for honesty). |
| **Unit** | **Headline only.** PhraseBank is single sentences with no summary field, so there's nothing to train summary on; serving headline+summary while training headline-only would be train/serve skew. The "longer context helps" revisit belongs to the **v3 weak-labelled-RSS run**, where `news.summary` is populated. |
| **Promotion rule** | Promote **iff `macro-F1 ≥ 0.692` (baseline 0.642 + 0.05) AND `bearish-F1 ≥ 0.525`**, both on `AllAgree`. The +0.05 absolute margin sits comfortably above GPU run-to-run variance (±0.01–0.02), so a pass means *clearly* better, not lucky — and it's a low bar a real transformer should clear (~0.75–0.85 macro expected), so failing it means the fine-tune isn't earning its complexity. No per-class-regression clause beyond the bearish floor (avoids over-constraining). |
| **Registry / mechanism** | Register the fine-tune as `finance-sentiment` **v2**, alias **`@challenger`**. On gate pass, **manually** move `@champion` v1→v2 (baseline demoted, kept as rollback). Automated promotion is **Stage 9**; Stage 4 reads the gate output and moves the alias by hand. |
| **Code shape** | New module **`finetune.py`** (transformers `Trainer`; same run-by-hand shape as `ingest`/`label`/`baseline`). **Shared scoring stays in `baseline.py`** — factor the score-out into `evaluate(y_true, y_pred) -> {macro_f1, bearish_f1, report}`; `finetune.py` imports it + `load_phrasebank`/`train_eval_split` so **both models are scored by identical code** and the comparison can't drift (move to a dedicated `eval.py` only if it grows). Reads the **DVC-versioned `data/phrasebank/`** from Stage 5 (no `train.parquet` yet — deferred to v3). Logs via **`mlflow.transformers.log_model`** (transformers flavor for Stage 7/BentoML) + register v2 `@challenger`. **Gate = an inline check** against the two constants printing pass/fail (Stage 9 formalises/automates it later). |

**Stage 4 code shape:** new `finetune.py` — load DVC `data/phrasebank/` via the baseline's `train_eval_split`, stratified 90/10 train/val split, tokenize headlines (`yiyanghkust/finbert-pretrain` @ pinned revision, `max_len=128`), HF `Trainer` (weighted CE, bf16, early stop on val macro-F1), score `AllAgree` once via the shared `evaluate()`, log params+metrics+model to MLflow, register `finance-sentiment` v2 `@challenger`, inline gate check vs `0.692`/`0.525`. Refactor `baseline.py` to expose `evaluate()`. New extra: `finetune = [torch, transformers, datasets, accelerate, mlflow]`.
...
| Decision | Resolution |
|---|---|
| **Tracker** | **MLflow 3.x** (not Aim). Already on the CV, so the new-skill budget goes to DVC/MinIO/Dagster instead; it ships a registry in the same tool and pairs natively with BentoML at Stage 7. Aim's only edge — "fully local" — MLflow matches, so it'd cost a new-tool tax for no extra self-hosting credit. |
| **Backend store** | **Reuse Postgres** — a dedicated `mlflow` database on the same instance Stages 1–2 use. The registry *forces* a DB backend (file store has no registry), so the choice is SQLite vs Postgres; reusing Postgres is rung-5 lazy, dodges SQLite write-locking once **Dagster (Stage 6)** runs concurrent jobs, and avoids a later backend migration. Consequence: MLflow runs as a real `mlflow server` process (Compose service at Stage 11), which Stages 6/7 want reachable anyway. **Implemented (2026-06-27):** there was no host Postgres, so the one instance lives in the Compose stack (`postgres` service, `finance_mlops` + `mlflow` DBs) rather than on the host — same "one instance, two DBs" decision, just containerised early. |
| **Object storage** | **MinIO now** — one S3-compatible service, two buckets (`mlflow-artifacts`, `dvc`), backing **both** the MLflow artifact store and the DVC remote. Functionally local dirs would carry Stage 5, but the README's stated purpose is learning modern MLOps tooling and §5 names MinIO+DVC as top-value — here the learning goal *is* the spec, so the ponytail "skip the object store" reflex is overridden deliberately. One Compose service; spares a later artifact-path migration + DVC re-push. |
| **DVC scope** | **Version `data/phrasebank/` → MinIO** (satisfies the Stage 2 "AllAgree frozen + DVC-versioned" decision; the anchor lives in that dir). **`train.parquet` deferred to Stage 4** (Stage 3 settled day-zero = single corpus, no built file). **Never DVC-version Postgres tables** (`news`/`weak_labels` stay in PG; DVC versions files). Real payoff now = establishing the DVC→MinIO pattern before Stage 4 brings churning data, not solving a versioning pain that exists today. |
| **MLflow / DVC division** | **MLflow owns** runs, params, metrics, **trained models**, eval report (artifact). **DVC owns** datasets. **Postgres owns** `news`/`weak_labels`. Models live in MLflow + registry, never double-stored in DVC. |
| **Registry** | One registered model **`finance-sentiment`** (ticker-blind, document-grain), versioned over time (baseline = v1, FinBERT = v2…). Promotion via **MLflow 3.x aliases `@champion`/`@challenger`** — *not* the deprecated 2.x stages (corrects the "staging → production" wording above). Serving (Stage 7) loads `models:/finance-sentiment@champion`, so promotion is an alias move with no redeploy. Baseline registered as **v1 + `@champion` manually**; the automated gate (promote `@challenger`→`@champion` only if it clears 0.642 macro-F1 / 0.525 bearish-F1) is **Stage 9's** job. Stage 5's payoff = standing up the `@champion` contract Stages 7 + 9 depend on. |
| **`baseline.py` integration** | **Explicit logging inside `baseline.py`** (no separate runner module — Dagster wraps these at Stage 6; same "run by hand" shape as `ingest`/`label`). **No `autolog`** — it logs training-time metrics, not the gate metric (macro-F1 + bearish-F1 on the held-out `AllAgree` anchor under the Stage 2 scoring rule), so it'd clutter the run and still leave the real numbers to log by hand. Log: params (`ngram_range`, `min_df`, `sublinear_tf`, `max_iter`, `class_weight`, train/eval sizes), metrics (gate numbers first-class), model (`mlflow.sklearn.log_model`, `registered_model_name="finance-sentiment"`), artifacts (`classification_report` + `stage3-baseline-report.md`); then set `@champion`. **`run()` still returns the float** so the Stage 9 gate can call it. **Re-run** the wired script to mint v1 through MLflow (deterministic → 0.642 reproduces). |

**Stage 5 code shape:** new Compose services `mlflow` (server, `--backend-store-uri postgresql:///mlflow`, `--default-artifact-root s3://mlflow-artifacts`) and `minio` (two buckets). `dvc init` + `dvc remote add` → MinIO `dvc` bucket, `dvc add data/phrasebank`, `dvc push`. `baseline.py` gains explicit MLflow logging + registration. New deps: `mlflow>=3`, `dvc[s3]`, `boto3` (S3 creds via env).
