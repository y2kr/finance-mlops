...
| Decision | Resolution |
|---|---|
| **Canonical rubric** | **PhraseBank-style "investor-polarity-of-the-text"**: polarity for an investor holding the mentioned company, judged from the headline alone, ignoring your own market view. Tie-breakers: factual/no clear direction → `neutral`; not about a tradable company/market → `irrelevant`. Written once to **`LABELING.md`** (carries a `rubric_version`) — the single source of truth quoted verbatim by the Ollama prompt and the Label Studio instructions. Rejected forward-return-defined labels (different latent target, would silently switch the task). |
| **Training corpus (sentiment)** | **Financial PhraseBank only for v1**, `sentences_75agree` subset (~3.4k, the literature-standard split → keeps "beat FinBERT" apples-to-apples). One subset loaded, no dedup logic. FiQA / SemEval-2017 T5 / TFNS **deferred** as a measured data-scaling experiment run through the pipeline (TFNS first — already 3-class; FiQA/SemEval last — continuous scores need invented thresholds = a second guideline = label noise). |
| **`irrelevant` seed** | **AG News, World + Sports categories only** (~1k), Business/Sci-Tech explicitly excluded (would teach "financial = irrelevant"). Known cold-start limitation: PhraseBank-vs-AG-News **style/register shortcut** — accepted, because in-domain RSS weak-labels + gold set wash it out once live data flows. Cold-start `irrelevant` is a throwaway bootstrap, not the final classifier. |
| **Live-label architecture** | **Strict two-role wall.** Ollama weak-labels → **training only** (volume). Label Studio gold set → **eval only**, never trained on (keeps the gate + NannyML/Evidently honest). Forward-return → **Stage-8 correlation panel, not a label**. |
| **Weak-labeller** | **`qwen2.5:14b-instruct`** (offline/hourly/batch → latency free, spend it on quality since the teacher caps the trained model's ceiling), tag+digest pinned. **Ollama structured output**, enum-constrained JSON `{label, confidence, rationale}`, temperature 0. Prompt quotes `LABELING.md` + 1–2 PhraseBank few-shots per class. The LLM catches financial-but-off-target junk → `irrelevant` that cold-start misses. |
| **Confidence handling** | LLM self-confidence is weakly calibrated → used coarsely only. v1: store `confidence`, **drop weak labels below 0.6** from training (one knob, no fancy sample-weighting). **Deferred (Stage 8/10):** low-confidence + weak-labeller-vs-deployed-model disagreement auto-populate the Label Studio queue (active learning). |
| **Gold set** | Sampled from **own RSS stream** (never held-out PhraseBank → no leakage), **stratified by weak-label** so `bearish` clears the gate's per-class floor (caveat: inherits weak-labeller bias). **~400 to start (~100/class).** Solo-annotated (single-annotator limitation acknowledged; you are the rubric author). **Split frozen-anchor + rolling-current** from day one: frozen = regression detector, rolling = honest live performance, refreshed on the retraining cadence. |
| **Label-drift control** | (1) `LABELING.md` versioned in git, prompt generated from it. (2) Every weak-label row stamped `(model_tag+digest, prompt_version, rubric_version)` and **immutable** — re-labelling produces a new DVC labelled-set version, never mutates in place; retraining selects a version explicitly. (3) Model+prompt pinned → upgrades are version bumps. (4) **Frozen-anchor gold set is the detector**: macro-F1 drop with no model change = silent redefinition; changing the rubric forces re-labelling the anchor — that cost is the feature. |
| **Phoenix** | **Skipped for v1.** A single-span batch labeller doesn't need a trace store; `{label, confidence, rationale, model_tag, prompt_version}` in Postgres + a Grafana panel covers the audit need with no new service. Named deferred *showcase* milestone — add when the labeller grows a multi-step pass or the trace-store talking point is wanted. |
